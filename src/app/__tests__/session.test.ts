...
describe('ProjectSession', () => {
  ...
  it('export strips caches, freshens metadata, and leaves untouched docs byte-identical', () => {
    const input = fixtureFiles()
    const s = ProjectSession.open(input)
    s.applyEdit(UUID_SCENE1, SCENE1_TEXT.replace('world', 'there'))
    const out = s.exportFiles()

    expect(out.has('docs.checksum')).toBe(false)
    expect(out.has('search.indexes')).toBe(false)
    expect(out.has('binder.autosave')).toBe(false)
    expect(out.has('binder.backup')).toBe(false)
    ...
    expect(new TextDecoder().decode(input.get('Baseline.scriv/docs.checksum')!)).toBe(
      'fake-checksum',
    )
  })
